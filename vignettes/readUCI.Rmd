...
The function `test_train_split` allows useres to split a dataset of their choice by test and train datasets. When performing classification methods, instead of splitting test and training data and separating variables into input and output data for class manually, we can use the `test_train_split` function.
